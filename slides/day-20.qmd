...
# Tuning
set.seed(1234)
cls_tune <- tune_grid(cls_wflow, resamples = cls_vfold, grid = cls_grid)
```
```{webr-r}
#| context: setup
p <- parameters(
...
# Finalize workflow
cls_tune_wflow <- finalize_workflow(cls_wflow, cls_best)
```
```{webr-r}
#| context: setup
# Visualize tuning results
...
```{webr-r}
# Collect metrics
cls_fit_cv |> collect_metrics()

# Confusion matrix
cls_fit_cv |> conf_mat_resampled(tidy = FALSE) |> autoplot(type = "heatmap")
```
...
## Interrogate: Model selection (x4) {.smaller .scrollable}
...
## Interrogate: Model tuning (x4) {.smaller .scrollable}
